...
class PaneHandler: ObservableObject
{
    ...
    init() {
        update(excluding: [AppleBundleIDs.Classroom.rawValue, // <-- excluding temporarily
            ...
            for file in files {
                if file.contains(".prefPane") {
                    ...
                    if id == AppleBundleIDs.AppleID.rawValue {
                        header.append(PaneItem(id: id, name: "Apple ID", image: image, path: path, pri: pri, section: sect))
                    } else if id == AppleBundleIDs.FamilySharing.rawValue {
                        header.append(PaneItem(id: id, name: "Family Sharing", image: image, path: path, pri: pri, section: sect))
                    } else {
